...
export const ScheduleController = new Elysia()
    ...
    return { data: db.data.schedule };}) // все данные выдает
  ...
  if (!schedule.employeeIds?.length || !schedule.startDate || !schedule.endDate) {
    return false;  // Проверяем что есть данные для фильтрации
  } return (
    schedule.employeeIds.some(id => requestedEmployeeIds.includes(id)) &&
